...
import SlotConfigManagement from './SlotConfigManagement';
import { apiClient } from '../services/apiClient';
import { useAuth } from '../auth/AuthProvider';
import { toast } from 'react-toastify';
import { formatReservationDate, formatTimestampSplit } from '../utils/dateUtils';

const AdminPanel: React.FC = () => {
    const [reservations, setReservations] = useState<Reservation[]>([]);
    const [loading, setLoading] = useState<boolean>(true);
    const [activeTab, setActiveTab] = useState<'reservations' | 'users' | 'database' | 'slot-config'>('users');
    ...
    // Using utility function to avoid timezone issues
    ...
    const confirmDeleteReservation = async () => {
        ...
        try {
            await apiClient.deleteReservation(reservationToDelete.id);
            // Refresh the current page data instead of just removing from local state
            await fetchReservations(currentPage, pageSize);
    ...
    const handleUpdateReservation = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Refresh the current page data instead of just updating local state
            await fetchReservations(currentPage, pageSize);
    ...
    const fetchReservations = async (page: number, limit: number) => {
        ...
        try {
            ...
            const response = await apiClient.getReservations({ limit, offset });
            
            // Map API reservations to local Reservation type
            const mappedReservations = response.reservations.map((apiReservation: any) => ({
                id: apiReservation.id,
                date: apiReservation.date,
                slot: apiReservation.slot as 'morning' | 'afternoon' | 'evening',
                creatorId: apiReservation.userId,
                creatorName: apiReservation.userName,
                notes: apiReservation.notes,
                createdAt: apiReservation.createdAt,
                updatedAt: apiReservation.updatedAt || apiReservation.createdAt
            }));
            
            // Sort by date ascending
            const sortedReservations = mappedReservations.sort((a: any, b: any) => a.date.localeCompare(b.date));
    ...
    const renderTabContent = () => {
        ...
        if (activeTab === 'slot-config') {
            return <SlotConfigManagement />;
        }
                    ...
                    >
                        <div className="flex items-center space-x-3">
                            ...
                            <div className="flex items-center space-x-2 text-sm text-slate-300">
                                ...
                                <span>{totalReservations} total reservations</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div style={{ 
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                                <label style={{ color: 'white', fontSize: '0.875rem' }}>Show:</label>
                                <select 
                                    value={pageSize} 
                                ...
                                <span style={{ color: 'rgba(203, 213, 225, 1)', fontSize: '0.875rem' }}>per page</span>
                            </div>
                            <div style={{ color: 'rgba(203, 213, 225, 1)', fontSize: '0.875rem' }}>
                                Showing {((currentPage - 1) * pageSize) + 1} to {Math.min(currentPage * pageSize, totalReservations)} of {totalReservations} reservations
                            </div>
                        </div>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                            <button
                                ...
                                style={{
                                    ...
                                    borderRadius: '0.25rem',
                                    border: '1px solid rgba(255, 255, 255, 0.2)',
                                    background: currentPage === 1 ? 'rgba(107, 114, 128, 0.5)' : 'rgba(59, 130, 246, 0.8)',
                                    color: currentPage === 1 ? 'rgba(156, 163, 175, 1)' : 'white',
                                    fontSize: '0.875rem',
                                ...
                                return (
                                    <button
                                        ...
                                        style={{
                                            ...
                                            borderRadius: '0.25rem',
                                            border: '1px solid rgba(255, 255, 255, 0.2)',
                                            background: pageNum === currentPage ? 'rgba(59, 130, 246, 1)' : 'rgba(51, 65, 85, 0.8)',
                                            color: 'white',
                                            fontSize: '0.875rem',
                                            cursor: 'pointer',
                            ...
                            <button
                                ...
                                style={{
                                    ...
                                    borderRadius: '0.25rem',
                                    border: '1px solid rgba(255, 255, 255, 0.2)',
                                    background: (currentPage === totalPages || !hasMore) ? 'rgba(107, 114, 128, 0.5)' : 'rgba(59, 130, 246, 0.8)',
                                    color: (currentPage === totalPages || !hasMore) ? 'rgba(156, 163, 175, 1)' : 'white',
                                    fontSize: '0.875rem',
                                    cursor: (currentPage === totalPages || !hasMore) ? 'not-allowed' : 'pointer'
                                }}
                            >
                                Next
                            </button>
    ...
    return (
        <>
                    ...
                    <div className="flex justify-center space-x-4 mb-6">
                        ...
                        <button 
                            style={{
                                ...
                                fontWeight: '500',
                                transition: 'all 0.2s',
                                background: activeTab === 'database' 
                                    ? 'linear-gradient(135deg, rgba(59, 130, 246, 0.9) 0%, rgba(37, 99, 235, 0.8) 100%)'
                                    : 'linear-gradient(135deg, rgba(71, 85, 105, 0.6) 0%, rgba(51, 65, 85, 0.5) 100%)',
                                border: '1px solid rgba(255, 255, 255, 0.2)',
                                color: 'white',
                                cursor: 'pointer',
                                boxShadow: activeTab === 'database' ? '0 4px 12px rgba(59, 130, 246, 0.3)' : '0 2px 8px rgba(0, 0, 0, 0.1)'
                            }}
                            onClick={() => setActiveTab('database')}
                            data-test="database-management-tab"
                            onMouseEnter={(e) => {
                                if (activeTab !== 'database') {
                                    e.currentTarget.style.background = 'linear-gradient(135deg, rgba(71, 85, 105, 0.8) 0%, rgba(51, 65, 85, 0.7) 100%)';
                                }
                        ...
                        <button 
                            style={{
                                ...
                                fontWeight: '500',
                                transition: 'all 0.2s',
                                background: activeTab === 'slot-config' 
                                    ? 'linear-gradient(135deg, rgba(59, 130, 246, 0.9) 0%, rgba(37, 99, 235, 0.8) 100%)'
                                    : 'linear-gradient(135deg, rgba(71, 85, 105, 0.6) 0%, rgba(51, 65, 85, 0.5) 100%)',
                                border: '1px solid rgba(255, 255, 255, 0.2)',
                                color: 'white',
                                cursor: 'pointer',
                                boxShadow: activeTab === 'slot-config' ? '0 4px 12px rgba(59, 130, 246, 0.3)' : '0 2px 8px rgba(0, 0, 0, 0.1)'
                            }}
                            onClick={() => setActiveTab('slot-config')}
                            data-test="slot-config-tab"
                            onMouseEnter={(e) => {
                                if (activeTab !== 'slot-config') {
                                    e.currentTarget.style.background = 'linear-gradient(135deg, rgba(71, 85, 105, 0.8) 0%, rgba(51, 65, 85, 0.7) 100%)';
                                }
            ...
            {/* Delete Reservation Modal - Positioned outside main container for proper centering */}
