...
import { resetDatabaseToInitialState } from '../utils/resetDatabase';
...
const DatabaseReset: React.FC = () => {
  ...
  const loadDatabaseStats = async () => {
    ...
    try {
      const dbStats = await apiClient.getDatabaseStats();
      setStats(dbStats);
    } catch (error) {
  ...
  if (authLoading) {
    return (
      <div className="mb-8">
        <div className="backdrop-blur-xl bg-orange-900/20 border border-orange-500/30 rounded-xl p-6">
          <div className="flex items-center justify-center">
            <div className="w-6 h-6 border-2 border-orange-500/30 border-t-orange-500 rounded-full animate-spin"></div>
            <span className="ml-2 text-orange-200">Loading...</span>
          </div>
        </div>
      </div>
    );
  ...
  // Don't render for non-authenticated users or non-admin users
  if (!user || !isAdmin) {
    return null;
  }
            ...
            <div className="mb-6">
              ...
              <ul className="text-sm text-slate-400 space-y-2 ml-4">
                <li className="flex items-center space-x-2">
                  ...
                  <span>Delete all {stats?.users || 0} users (including current admin)</span>
                </li>
                <li className="flex items-center space-x-2">
                  <span className="w-1.5 h-1.5 bg-red-400 rounded-full"></span>
                ...
                <li className="flex items-center space-x-2">
                  ...
                  <span>Show setup wizard after refresh (you will be logged out)</span>
                </li>
              </ul>
            </div>
