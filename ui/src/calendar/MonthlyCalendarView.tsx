...
const MonthlyCalendarView: React.FC<MonthlyCalendarViewProps> = ({
  ...
  const [tooltip, setTooltip] = useState<{ visible: boolean; x: number; y: number; content: string; showBelow?: boolean }>({ visible: false, x: 0, y: 0, content: '' });
        ...
        {days.map(day => {
          ...
          return (
            <div
              ...
              onMouseEnter={(e) => {
                if (dayReservations.length > 0) {
                  ...
                  setTooltip({
                    visible: true,
                    x: e.clientX,
                    y: e.clientY - 15, // Position slightly above the mouse pointer
                    content: tooltipContent,
                    showBelow: false // Always show above
                  });
                }
              }}
              onMouseMove={(e) => {
                if (dayReservations.length > 0 && tooltip.visible) {
              ...
              onMouseLeave={() => setTooltip({ visible: false, x: 0, y: 0, content: '', showBelow: false })}
