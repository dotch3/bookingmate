...
export const SlotCell: React.FC<SlotCellProps> = ({ date, slot, reservations, user, maxCapacity = 2, onReservationChange }) => {
  ...
  const userReservation = slotReservations.find(res => res.userId === user?.id);
  const isOwner = !!userReservation;
  const isSlotFull = slotReservations.length >= maxCapacity;
  ...
  const handleSlotClick = async () => {
    ...
    try {
      ...
      if (userReservation) {
        await deleteReservation(userReservation.id, user.id);
        toast.success('Reservation cancelled successfully');
      } else {
        if (isSlotFull) {
          toast.error(`This slot is full (maximum ${maxCapacity} reservations)`);
          return;
        }
      ...
      // Refresh reservations immediately after creating/deleting
  ...
  const getSlotLabel = () => {
    // Handle legacy slot names with time ranges
